...
export default function PhotoUploader({ addedPhotos, setAddedPhotos }) {
  ...
  const addPhotoByLink = async (ev) => {
    ...
    const { data: filename } = await axios.post("/api/places/upload-by-link", {
      link: photoLink,
    });
  ...
  const makeThumbnail = (ev, filename) => {
    ev.preventDefault();
          ...
          addedPhotos.map((link) => (
            <div key={link} className="h-32 flex relative">
              <img
                ...
                src={"http://localhost:4153/uploads/" + link}
                alt="hotelImg"
              />
              <button
              ...
              <button
                ...
                onClick={(ev) => makeThumbnail(ev, link)}
              >
                {link === addedPhotos[0] && (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
